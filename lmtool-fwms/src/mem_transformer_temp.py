    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        if self.update_mode == 'soft':
            ...
            if self.training is True:
                pi_new = torch.sum(attn_prob_full, dim=(0,2), keepdim=True)/(h.size(1) * h.size(0))
...
class MemTransformerLM(nn.Module):
    ...
    def _forward(self, dec_inp, mems=None, carry_over_fast_weight=False):
        ...
        elif self.attn_type in [2, 4, 5, 6, 7, 10, 14, 16, 200, 201, 202]:  # absolute
            if self.no_pos:
                core_out = self.drop(word_emb)
            else:
