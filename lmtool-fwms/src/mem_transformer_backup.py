...
class HDPAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, n_global_head, dropatt=0, 
        ...
        self.q_net = nn.Linear(d_model, self.n_global_head * d_head, bias=False) # self.n_global_head = 4
        self.k_net = nn.Linear(d_model, self.n_head * d_head, bias=False) #self.n_head = 8
        self.v_net = nn.Linear(d_model, self.n_head  * d_head, bias=False)
        self.hdp_net_q = nn.Linear(self.n_global_head, self.n_head, bias=False)
        # self.hdp_net_k = nn.Linear(self.n_global_head, self.n_head, bias=False)
    ...
    def forward(self, h, attn_mask=None, mems=None,
        ...
        head_q = head_q.transpose(2,3)
        head_q = self.hdp_net_q(head_q)
        head_q = head_q.transpose(2,3)
        
#         head_k = head_k.transpose(2,3)
#         head_k = self.hdp_net_k(head_k)
#         head_k = head_k.transpose(2,3)
